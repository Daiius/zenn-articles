...
なのでメインの遊び方として「サカバンバスピス」が出るまで顔生成ボタンを連打してもらうというのがあるのですが、**致命的な見落としに気づきました**。

ある日、自分のサイト名で Google 検索してみたところ、**ユーザーがシェアした「サカバンバスピス」seed 付きの URL が検索結果に表示されている**ことに気づきました。

![Google 検索結果に seed 付き URL が表示されている様子](/images/sacapis-google-search-console/search-result-indexed.png)
*TODO: Google 検索結果のスクリーンショットをここに配置*

本来、検索結果に出てほしいのはトップページ（`https://sacapis2026.faveo-systema.net/`）だけで、`?seed=12345` のような個別の URL は検索に出てほしくありません。しかし当時の実装では、**`noindex` も `canonical` も設定していませんでした。**

検索結果に載るようなパブリックな Web アプリを個人開発し始めたのは最近で、Google 検索の存在を軽視していました...
...
## 対応その1: Google Search Console から検索結果を削除

まず、既にインデックスされてしまった URL を検索結果から消すために、Google Search Console の「削除」機能を使いました。

![Search Console の削除リクエスト画面](/images/sacapis-google-search-console/search-console-removal.png)
*TODO: Search Console の「削除」画面のスクリーンショットをここに配置*

Google Search Console には「一時的な削除」という機能があり、特定の URL やプレフィックスに一致する URL を約6ヶ月間、検索結果から非表示にすることができます。

![削除リクエスト送信後の確認画面](/images/sacapis-google-search-console/search-console-removal-confirmed.png)
*TODO: 削除リクエスト送信後の確認画面のスクリーンショットをここに配置*

:::message alert
この機能はあくまで**一時的な非表示**です。6ヶ月後には再びインデックスされる可能性があるため、根本的な対策（noindex や canonical の設定）が別途必要です。
:::

## 対応その2: Google Search Console の所有権確認

Search Console でサイトを管理するためには、サイトの所有権を確認する必要があります。確認方法はいくつかありますが、今回は HTML メタタグを埋め込む方法を選びました。
...
| 用途 | URL | 例 |
|------|-----|----|
| `og:url`（SNS シェア用） | seed 付き | `https://example.com/?seed=12345` |
| `canonical`（SEO 用） | seed なし | `https://example.com/` |
...
- **動的に URL が生成されるアプリでは、公開前に `noindex` / `canonical` を設定すべき**。クエリパラメータで内容が変わるページは、意図せずインデックスされるリスクがある

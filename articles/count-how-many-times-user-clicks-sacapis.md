---
title: "「全員で何回クリックした？」を実装するのって案外難しい"
emoji: "🔢"
...
import { redis } from "./redis"; // ioredis で Redis クライアントを作成
...
この方法には問題があります。`seq` を偽造したリクエストで簡単に突破できてしまいます。

クライアント側で `seq` を生成している以上、開発者ツールから任意の値を送信可能です。例えば：
- `seq` を 1, 2, 3... と順番に送れば、いくらでもカウントを増やせる
- `clientId` も自由に変更できるため、複数ユーザを装える

一方、今回のトークン方式では：
- **トークンはサーバー側で生成**（クライアントは偽造不可）
- **発行時刻もサーバー側で記録**（改ざん不可）
- **使い捨てなので再利用も不可**
- **時間ベースの検証**で不自然なカウントを拒否
...
「全ユーザのクリック回数をカウントする」というシンプルな要件でも、連打対策・不正リクエスト対策・パフォーマンスを考慮すると、意外と複雑な設計が必要になります。

今回紹介した「**短命トークン + 時間制限 + 最大値制限**」の組み合わせは：

**✅ アーキテクチャの利点:**
- クライアント側でカウントを蓄積（リクエスト数削減）
- サーバー側で検証（不正防止）
- Redis でトークン管理（高速・自動削除）
- メモリ内カウント + 定期保存（高速・永続化）

**✅ セキュリティ面:**
- サーバー発行のトークンで改ざん防止
- 時間ベースの検証で不自然なリクエストを拒否
- 不正リクエストにも成功レスポンスを返して攻撃者を欺く

という利点があり、**ゆるく、でもそこそこ正確に**カウントする用途には有効な選択肢の一つだと考えています。

**🫠 既知の課題と対策:**

もちろん完璧ではなく、以下のような課題もあります：
- トークン発行エンドポイントへの DDoS 攻撃
  → レートリミット、WAF での防御を検討
- 設定値のギリギリを攻めた不正カウント
  → ログ監視、異常検知、動的な閾値調整
- Redis のメモリ枯渇
...
実際の本番環境では、これらの対策を追加しつつ、数値を調整しながら運用することをお勧めします。

要件に応じて適した手法を選び、実験しながら調整していく。それが現実に合わせたベストな選択につながると考えています。

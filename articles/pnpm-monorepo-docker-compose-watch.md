...
graph TB
    ...
    subgraph I["Dockerfile.dev 方式：イメージ内に閉じる"]
...
| 観点 | Before（volume mount） | After（compose watch） |
| --- | --- | --- |
...
ここまで書いておいて何ですが、執筆中もずっと引っかかっていた疑問があります。モノレポを丸ごとイメージに焼くのは雑では？　パッケージごとに必要な依存だけ `pnpm install --filter` した最小イメージに分けるほうが効率的なのでは？　というものです。

結論から言うと、**開発環境に限っては、分けるほど損をする**と考えています。「効率」を分解すると、日々の編集→反映は watch の sync なのでどの構成でも変わらず、差が出るのは「依存追加時の再ビルド」と「ディスク」くらいです。

| 観点 | 全部入り1枚（現行） | パッケージ別の最小イメージ |
| --- | --- | --- |
...
効きそうで効かないのが「依存追加時」です。pnpm の lockfile はワークスペース共通の1枚なので、frontend に依存を足すと backend の最小イメージも `COPY pnpm-lock.yaml` の層から無効化されます。分けても再ビルドの回数は減らず、ビルドが N 本に増えるだけでした。さらに本記事の構成では frontend が backend の型を import しているため、「frontend の最小イメージ」にも結局 backend のソースが要ります。最小化しようとするほど、モノレポ内の依存関係を Dockerfile に手で書き写すことになります。

「`node_modules` だけ別イメージに分離する」案も考えましたが、これは manifest 先行 COPY → install という**レイヤーキャッシュが既にやってくれていることの手動化**でした。ソースをいくら変えても install 層は再利用されるので、切り出して得るものがほぼありません。（さらに遡って「node はホストで動かし、コンテナは DB だけ」という割り切りもありますが、環境差を消したいという出発点に戻ってしまうので採っていません。）

なお、これはあくまで開発環境の話です。本番イメージでは不要なソースや devDependencies を載せない価値が大きく、`pnpm deploy` や Turborepo の `prune` で最小化する意味が出てきます。**dev と prod では最適化の軸が違う**、というのがこの試行錯誤での学びでした。
...
volume mount 方式から `docker compose watch` 方式へ移したことで、

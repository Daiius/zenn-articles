...
monorepo を丸ごと焼き込んだ単一イメージ——いわば **mono-image**——を作る `Dockerfile.dev` と、差分を届ける watch。どちらが欠けても成立しない対の構成なので、本記事では**モノイメージ＋watch 方式**と呼ぶことにします。
...
graph TB
    ...
    subgraph I["モノイメージ＋watch 方式：イメージ内に閉じる"]
...
| 観点 | Before（volume mount） | After（モノイメージ＋watch） |
| --- | --- | --- |
...
volume mount 方式からモノイメージ＋watch 方式へ移したことで、

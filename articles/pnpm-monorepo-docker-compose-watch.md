...
```mermaid
flowchart LR
  subgraph host["ホスト（pnpm monorepo・node_modules なし）"]
    direction TB
    man["package.json 群<br/>pnpm-lock.yaml"]
    src["frontend/ backend/ worker/<br/>のソース"]
  end
  ...
  subgraph image["mono-image（/workspace）"]
    ...
    subgraph pkgs[" "]
      ...
      subgraph fe["frontend/"]
        feNM["node_modules<br/>（中身はほぼ symlink）"]
      end
      subgraph be["backend/"]
        beNM["node_modules"]
      end
    ...
    store["node_modules/.pnpm<br/>（依存の実体）"]
    feNM -. ".pnpm を指す" .-> store
    beNM -. symlink .-> store
    woNM -. symlink .-> store
    feNM -. "workspace:* は本体を指す" .-> be
  end

  man -- "COPY → pnpm install" --> store
  src -- "COPY（焼き込み）" --> pkgs
  src == "開発中は watch が差分を sync" ==> pkgs
```
...
#### 図案：volume mount だと symlink の網が境界を跨ぐ（置き場所検討中・本文図と対になる構図）

```mermaid
flowchart LR
  subgraph host["ホスト（pnpm monorepo）"]
    direction TB
    hsrc["frontend/ backend/ worker/<br/>のソース"]
  end

  subgraph cont["コンテナ（/workspace）"]
    direction TB
    subgraph fe["frontend/（bind mount）"]
      feNM["node_modules<br/>（volume ②でマスク）"]
    end
    subgraph be["backend/（bind mount）"]
      beNM["node_modules<br/>（volume ③でマスク）"]
    end
    subgraph wo["worker/（bind mount）"]
      woNM["node_modules<br/>（volume ④でマスク）"]
    end
    subgraph vol1["named volume ①"]
      store["node_modules/.pnpm<br/>（依存の実体）"]
    end
    feNM -. "volume の境界を跨ぐ" .-> store
    beNM -. symlink .-> store
    woNM -. symlink .-> store
    feNM -. "workspace:* は volume と<br/>bind mount の境界を跨ぐ" .-> be
  end

  hsrc == "bind mount" ==> fe & be & wo

  linkStyle 0,1,2,3 stroke:#e66,stroke-width:2px
```
